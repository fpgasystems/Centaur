...
*/
bool HWService::allocate_workspace(uint64_t size){
  MSG("Allocate Workspace.... size:"<<size);
...
bool HWService::init()
{
  ...
  m_SPLService->StartTransactionContext(TransactionID(), m_AFUCTXVirt, 100);
  m_Sem.Wait();

  if(m_AFUDSMSize == 0) return false;
  return true;
}
